...
CREATE TABLE IF NOT EXISTS `mydb`.`RejectedTrainingMessage` (
  ...
  `description` VARCHAR(45) NULL,
  `RejectedTrainingMessage_idRejectedTrainingMessage` INT NOT NULL,
  `Student_serialNumber` VARCHAR(45) NOT NULL,
  PRIMARY KEY (`idRejectedTrainingMessage`),
  INDEX `fk_RejectedTrainingMessage_RejectedTrainingMessage1_idx` (`RejectedTrainingMessage_idRejectedTrainingMessage` ASC),
  INDEX `fk_RejectedTrainingMessage_Student1_idx` (`Student_serialNumber` ASC),
  CONSTRAINT `fk_RejectedTrainingMessage_RejectedTrainingMessage1`
    FOREIGN KEY (`RejectedTrainingMessage_idRejectedTrainingMessage`)
    REFERENCES `mydb`.`RejectedTrainingMessage` (`idRejectedTrainingMessage`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
